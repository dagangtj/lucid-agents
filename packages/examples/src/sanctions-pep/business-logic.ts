 ...
 * 
 * Core data transforms, scoring, and ranking behavior.
 * Extracted from test file to avoid production code depending on tests.
 */
...
/**
 * Calculate match confidence between entity name and sanctions list entry
 */
...
/**
 * Build ownership chain from entity name and records
 */
 ...
 */
export function calculateFreshnessMetadata(
  lastUpdated: Date,
  ...
  // Use configurable refresh interval or default to 2 hours
  const interval = refreshIntervalHours ?? 2;
  const nextRefresh = new Date(
    lastUpdated.getTime() + interval * 60 * 60 * 1000
  );
